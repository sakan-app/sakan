...
- الطبقة الحالية مستقلة عن المزوّد (`src/lib/billing/provider.server.ts`) وتدعم الدفع اليدوي.
- تفعيل Stripe يتطلّب **المفتاح السري** (`sk_test_...`) — المفتاح الذي تم إرساله كان المفتاح العلني (`pk_test_...`) مكرّرًا، لذلك لم يُفعَّل الربط بعد.
